...
- (NSString*)systemUserAgent
{
    ...
    NSString *kOMDbUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
    
    return [[NSString stringWithFormat:@"%@/%s (%@)",kOMDbUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a OMDb app using the node.js CLI
