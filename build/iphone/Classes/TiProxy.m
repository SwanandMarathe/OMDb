...
//Rare exceptions to indicate a bug in the _omdb code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_omdb = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_omdb]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.omdb.module"
